...
#   LastEd: October 2024
#    Brief: 
#         Launches a shoreside, which receives a connection
#         from a node. A node being a simulated vehicle or a 
#         physical vehicle. 
#--------------------------------------------------------------
...
#--------------------------------------------------------------
nsplug meta_shoreside.moos targs/targ_shoreside.moos -i -f WARP=$TIME_WARP  \
       IP_ADDR=$IP_ADDR       PSHARE_PORT=$PSHARE_PORT                \
       MOOS_PORT=$MOOS_PORT   REGION=$REGION                          \
       FORCE_IP=$FORCE_IP     MISSION_NAME=$MISSION_NAME              \
       VNAMES=$VNAMES         HIGH_RES=$HIGH_RES
